...
function mostrarProyectosUserAdmin($idUser)
{
    try {
        $count = 0;
...
function mostrarNombreUsuario($idUser){
    try {
        $user = obtenerUsuarioUnicoId($idUser)->fetch();

        echo $user['usuario'];

    } catch (PDOException $e) {
        echo "Error obtenerNombreUsuario: " . $e->getMessage();
    }
}